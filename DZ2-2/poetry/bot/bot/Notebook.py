    ...
    def _check_tags(self, tags):
        ...
        for tag in tmp:
            ...
            if len(tg) > self.limit_tag:
                raise ValueError(
                    get_message("long_tag")
                    .replace("{tag}", tg)
                    .replace("{limit}", self.limit_tag)
                )
        ...
        if tag:
            if self.exists_tag(tag):
                if is_short:
                    ...
                    res = "{:>5} | {:<15} | {:<80}".format(
                        self.numb, tag.lower(), self.content
                    )
        else:
            tags = self._form_tags()
            for numb, tg in enumerate(tags, 0):
                if numb == 0:
                    res = res + "{:>5} | {:<15} | {:<80}".format(
                        self.numb, tg, self.content
                    )
                else:
                    res = res + "\n" + "{:>5} | {:^15} | {:<80}".format(" ", tg, " ")
        return res
...
class NoteBook(UserList):
    ...
    def edit_note(self, tag: str | int, new_text: str):
        ...
        if len(found_notes) == 1:
            ...
            lst = (
                get_message("tag_found")
                .replace("{tag}", tag)
                .replace("{count}", str(len(found_notes)))
                + "\n"
                + sep
            )
    ...
    def search_by_substring(self, mask: str):
        ...
        if found_notes:
            lst = (
                get_message("note_found").replace("{count}", str(len(found_notes)))
                + "\n"
                + top
            )
            for note in found_notes:
