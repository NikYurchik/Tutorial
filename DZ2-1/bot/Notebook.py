...
messages = {
    'en': {
        ...
        'clear?': 'Очистить всю книгу заметок (Y - да / N - нет)? ',
...
class NoteBook(UserList):
    ...
    def __init__(self, interface, note: Note | str = None, tags: list | str = None):
        ...
        botsetting = Bot_setting(interface)
        if note:
    ...
    def _search_notes_by_tag(self, tag: str):
        found_notes = []
        for note in self.data:
            if note.exists_tag(tag):
    ...
    def edit_note(self, tag: str, new_text: str):
    ...
    def search_by_tag(self, tag: str):
        ...
        if found_notes:
            ...
            lst = get_message('all_note').replace("{tag}", tag)
        return lst
    ...
    def search_by_substring(self, mask: str):
        ...
        if found_notes:
            ...
            lst = get_message('all_note').replace("{mask}", mask)
        return lst

    def view_notes(self, tag: str = None):
        if tag:
            lst = get_message('all_note').replace("{tag}", tag) + '\n' + sep
            found_notes = self._search_notes_by_tag(tag)
        else:
